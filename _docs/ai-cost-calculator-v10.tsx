...
async function fetchWithRetry(url, options = {}, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Don't retry on 4xx errors (invalid key, etc)
...
const apiCache = new APICache(3600000); // 1 hour TTL
...
export default function AITokenCalculator() {
  ...
  // Map model identifiers to our internal keys
  const modelMapping = {
    'gpt-4': 'gpt-4',
    ...
    try {
      ...
      if (provider === 'openai') {
        ...
        if (cachedData) {
          ...
          const response = await fetchWithRetry('https://api.openai.com/v1/usage', {
            method: 'GET',
            headers: {
          ...
          usageData = parseOpenAIUsage(data);
        ...
        // For now, we'll prompt them to use the quick calculator
        setError('Anthropic usage API coming soon. Please use Quick Calculator with your estimated monthly tokens.');
        setLoading(false);
        return;
      }

      // Calculate costs for their actual usage
      ...
      modelBreakdown.forEach(model => {
        const modelData = models[model.key];
        if (modelData) {
          currentCost += (model.inputTokens / 1000000) * modelData.input;
          currentCost += (model.outputTokens / 1000000) * modelData.output;
        }
      });

      // Calculate alternatives using their total token count
      const alternatives = Object.entries(models)
        .map(([key, model]) => {
          const altCost = (totalInputTokens / 1000000) * model.input + 
                         (totalOutputTokens / 1000000) * model.output;
          return {
            key,
            model,
            cost: altCost,
      ...
      setError(err.message || 'Failed to analyze usage. Please check your API key and try again.');
    } finally {
      setLoading(false);
    }
  };

  const parseOpenAIUsage = (data) => {
    // OpenAI usage API returns data in different formats
    // This is a simplified parser - actual API structure may vary
    const now = new Date();
    ...
    // Parse usage data (structure depends on OpenAI's actual API response)
    if (data.data && Array.isArray(data.data)) {
      data.data.forEach(entry => {
        const entryDate = new Date(entry.timestamp || entry.date);
        if (entryDate >= thirtyDaysAgo) {
          const modelName = entry.model || entry.snapshot_id;
          const inputTokens = entry.n_context_tokens_total || entry.input_tokens || 0;
          const outputTokens = entry.n_generated_tokens_total || entry.output_tokens || 0;

          totalInputTokens += inputTokens;
          totalOutputTokens += outputTokens;

          // Map to our model keys
          const mappedKey = modelMapping[modelName] || 'gpt-4';
          
          if (!modelUsage[mappedKey]) {
            modelUsage[mappedKey] = { inputTokens: 0, outputTokens: 0 };
          }
          modelUsage[mappedKey].inputTokens += inputTokens;
          modelUsage[mappedKey].outputTokens += outputTokens;
        }
      });
    }

    const modelBreakdown = Object.entries(modelUsage).map(([key, usage]) => ({
      key,
      name: models[key]?.name || key,
      inputTokens: usage.inputTokens,
      outputTokens: usage.outputTokens,
      percentage: ((usage.inputTokens + usage.outputTokens) / (totalInputTokens + totalOutputTokens) * 100).toFixed(1)
    }));
  ...
  const shareOnTwitter = () => {
    if (!results) return;
    const bestSavings = results.alternatives[0];
    const savingsAmount = bestSavings.savings > 0 ? formatCurrency(bestSavings.savings) : '$0';
    const text = `I just checked my AI token costs 🤯\n\nCurrently spending: ${formatCurrency(results.currentCost)}/mo\nCould save: ${savingsAmount}/mo by switching to ${bestSavings.model.name}\n\nCheck yours:`;
  ...
  const downloadReport = () => {
    ...
    // Create a shareable summary card
    ...
    ctx.fillText('AI Token Cost Analysis', 60, 100);
    
    // Current Cost Box
    ctx.fillStyle = 'rgba(255, 255, 255, 0.15)';
    ctx.fillRect(60, 150, 520, 180);
    ctx.fillStyle = '#FFFFFF';
    ctx.font = '24px system-ui';
    ctx.fillText('Currently Spending', 80, 190);
    ctx.font = 'bold 72px system-ui';
    ctx.fillText(formatCurrency(results.currentCost), 80, 270);
    ctx.font = '20px system-ui';
    ctx.fillText('per month', 80, 305);
    
    // Potential Savings Box
    const bestSavings = results.alternatives[0];
    if (bestSavings.savings > 0) {
      ctx.fillStyle = 'rgba(16, 185, 129, 0.3)';
      ctx.fillRect(620, 150, 520, 180);
      ctx.fillStyle = '#10B981';
      ctx.font = '24px system-ui';
      ctx.fillText('Potential Savings', 640, 190);
      ctx.font = 'bold 72px system-ui';
      ctx.fillText(formatCurrency(bestSavings.savings), 640, 270);
      ctx.font = '20px system-ui';
      ctx.fillText(`Switch to ${bestSavings.model.name}`, 640, 305);
    }
    
    // Token Usage
    ctx.fillStyle = 'rgba(255, 255, 255, 0.15)';
    ctx.fillRect(60, 370, 1080, 100);
    ctx.fillStyle = '#FFFFFF';
    ctx.font = '20px system-ui';
    ctx.fillText(`Input: ${formatTokens(results.inputTokens)} tokens`, 80, 410);
    ctx.fillText(`Output: ${formatTokens(results.outputTokens)} tokens`, 80, 445);
    
    if (results.mode === 'exact' && results.modelBreakdown) {
      ctx.fillText('Models: ' + results.modelBreakdown.map(m => m.name).join(', '), 400, 427);
    } else if (results.currentModel) {
      ctx.fillText(`Model: ${results.currentModel.name}`, 400, 427);
    }
    
    // Footer
    ctx.font = 'bold 28px system-ui';
    ctx.fillText('aiburn.howstud.io', 60, 560);
    ctx.font = '18px system-ui';
    ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.fillText('Free AI Token Cost Calculator', 60, 590);
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `ai-cost-report-${new Date().toISOString().split('T')[0]}.png`;
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 relative group">
                        <button
                          disabled
                          className="w-full py-3 px-6 rounded-xl font-semibold bg-gray-100 text-gray-400 cursor-not-allowed opacity-60"
                        >
                          Anthropic
                        </button>
                        <div className="absolute bottom-full mb-2 right-0 bg-gray-900 text-white text-xs px-3 py-2 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-auto z-10">
                          Anthropic API doesn't offer usage endpoint yet.{' '}
                          <a 
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-3xl shadow-xl p-8 text-white">
                  <h2 className="text-2xl font-bold mb-6">
                    {results.mode === 'exact' ? 'Your Last 30 Days (Actual Usage)' : 'Your Current Costs'}
                  </h2>
                  ...
                  {results.mode === 'exact' && results.modelBreakdown ? (
                    <div className="bg-white/10 rounded-xl p-4">
                      <div className="font-bold text-lg mb-3">Model Usage Breakdown</div>
                      <div className="space-y-2">
                        {results.modelBreakdown.map((model, idx) => (
                          <div key={idx} className="flex items-center justify-between text-sm">
                            <span>{model.name}</span>
                            <span className="font-semibold">{model.percentage}% ({formatTokens(model.inputTokens + model.outputTokens)} tokens)</span>
                          </div>
                        ))}
                      </div>
                    </div>
